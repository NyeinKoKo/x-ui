...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "x-ui Stopped successfully with xray"
        else
...
restart() {
    ...
    check_status
    if [[ $? == 0 ]]; then
        LOGI "x-ui Restarted successfully with xray"
    else
...
ssl_cert_issue() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -ne 0 ]; then
            ...
            LOGI "The certificate is successfully installed and automatic updates are enabled...."
        fi
